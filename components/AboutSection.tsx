...
export default function AboutSection() {
  ...
  const cards = [
    { icon: GraduationCap, color: "from-violet-500 to-purple-600", ...ab.cards.education },
    { icon: MapPin, color: "from-cyan-500 to-blue-600", ...ab.cards.location },
    { icon: Briefcase, color: "from-emerald-500 to-teal-600", ...ab.cards.status },
    { icon: Calendar, color: "from-orange-500 to-amber-600", ...ab.cards.experience },
  ];
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          <div className="grid grid-cols-1 gap-4">
            {cards.map(({ icon: Icon, color, title, main, sub, extra }, i) => (
              <ScrollReveal key={title} direction="up" delay={i * 0.1}>
                <div className="glass gradient-border rounded-2xl p-5 flex items-start gap-4 card-hover">
                  ...
                  <div>
                    <p className="text-xs font-semibold tracking-widest uppercase mb-0.5" style={{ color: "var(--text-muted)" }}>{title}</p>
